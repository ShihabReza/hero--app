...
const Tranding = ({trandingApp}) => {
    ...
    return (
        <div>
            ...
            <p className=' text-center mt-2 mb-10'>Explore All Trending Apps on the Market developed by us</p>
            <div  className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-w-screen-2xl mx-auto px-4">
            
            {
                trand.map(trand=><Trand trand={trand}></Trand>)
            }
        </div>
        <div className='text-center mt-5 mb-5'>
           <Link to='/Apps'> <button className='btn border-indigo-300 text-white bg-indigo-500 hover:bg-indigo-600 px-6 py-2 '>Show All</button></Link>
        </div>
        </div>
    );
};
